...
#include <string>
#include "data.h"
#include "xtime_l.h" // Xilinx hardware timer library
...
int main() {
    ...
    cout << "=== CIFAR-10 ARM PS Baseline (Hardware Timers) ===" << endl;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // Calculate time: (Ticks / (Ticks per Second)) * 1000 for ms
        double dur_ms = 1000.0 * (double)(tEnd - tStart) / (double)COUNTS_PER_SECOND;
        total_ms += dur_ms;
